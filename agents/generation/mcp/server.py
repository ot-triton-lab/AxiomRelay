...
try:
    from .proof_context import ProofParseError, parse_blueprint
except ImportError:  # pragma: no cover - direct module execution
    from proof_context import ProofParseError, parse_blueprint
...
try:
    from .review_client import (
        ...
        continuous_round_finish as _adapter_continuous_round_finish,
        continuous_round_status as _adapter_continuous_round_status,
...
try:
    from .verification_client import (
        _profile_endpoint,
        _endpoint_uses_local_lifeline,
        _validate_verifier_profile,
        _verification_caller_binding,
        _verification_pass_identity,
...
ARXIV_API_URL = os.getenv(
    "RETHLAS_ARXIV_API_URL",
    "https://export.arxiv.org/api/query",
)
ARXIV_HTML_URL_TEMPLATE = os.getenv(
    "RETHLAS_ARXIV_HTML_URL_TEMPLATE",
...
MAX_ARXIV_METADATA_BYTES = 65_536
MAX_ARXIV_HTML_BYTES = 2_000_000
MAX_ARXIV_PRIMARY_EXCERPT_BYTES = 24_000
ARXIV_ID_RE = re.compile(
    r"^(?:[0-9]{4}\.[0-9]{4,5}|[A-Za-z][A-Za-z0-9.-]*/[0-9]{7})(?:v[1-9][0-9]*)?$"
)
...
MAX_MEMORY_SEARCH_QUERY_UTF8_BYTES = 8_192
MAX_MEMORY_SEARCH_RESPONSE_UTF8_BYTES = 60_000
MAX_OMITTED_IDS = 100
...
ROUND_FAILURE_SYNTHESIS_SCHEMA = "rethlas_round_failure_synthesis_v1"
MAX_ROUTE_TERMINAL_REPORT_BYTES = 16_384
...
_RESOLVED_COST_POLICY_JSON_ENV = "RETHLAS_RESOLVED_COST_POLICY_JSON"
_RESOLVED_COST_POLICY_SHA256_ENV = "RETHLAS_RESOLVED_COST_POLICY_SHA256"
_RESOLVED_COST_POLICY_SCHEMA = "rethlas_resolved_cost_policy_v1"
...
def _read_bounded_external_bytes(
...
def _fetch_bounded_external_bytes(
    url: str,
    *,
    timeout_seconds: int,
    maximum_bytes: int,
    ...
    try:
        response.raise_for_status()
        body = _read_bounded_external_bytes(
            response,
...
def _arxiv_base_id_key(arxiv_id: str) -> str:
    ...
    return re.sub(r"v[1-9][0-9]*$", "", _normalize_arxiv_id(arxiv_id)).casefold()
...
def _arxiv_atom_entry(raw: bytes, *, expected_arxiv_id: str) -> Dict[str, str]:
    ...
    entry_match = re.search(r"(?is)<entry\b[^>]*>(.*?)</entry>", text)
    if entry_match is None:
        raise ValueError("arXiv metadata has no entry")
    entry = entry_match.group(1)

    def field(name: str) -> str:
        match = re.search(rf"(?is)<{name}\b[^>]*>(.*?)</{name}>", entry)
        if match is None:
            raise ValueError(f"arXiv metadata has no {name}")
        without_tags = re.sub(r"(?s)<[^>]+>", " ", match.group(1))
        return " ".join(html.unescape(without_tags).split())

    entry_url = field("id")
    entry_id_match = re.fullmatch(
    ...
    if _arxiv_base_id_key(entry_arxiv_id) != _arxiv_base_id_key(
        expected_arxiv_id
    ):
        raise ValueError("arXiv metadata entry id differs from the requested id")

    published = field("published")
    initial_submission_date = _normalize_iso_date(
        published[:10], label="arXiv published date"
    )
    if initial_submission_date is None:  # pragma: no cover - field() is nonempty
        raise ValueError("arXiv metadata has no published date")
    return {
        "metadata_arxiv_id": entry_arxiv_id,
        "title": field("title"),
        "published": published,
...
def _arxiv_atom_entries(
    raw: bytes, *, expected_arxiv_ids: List[str]
) -> List[Dict[str, str]]:
    ...
    normalized_expected = [
        _normalize_arxiv_id(arxiv_id) for arxiv_id in expected_arxiv_ids
    ]
    ...
    try:
        ...
        raise ValueError("arXiv metadata is not UTF-8") from error
    entry_blocks = re.findall(r"(?is)<entry\b[^>]*>.*?</entry>", text)
    if not entry_blocks:
        raise ValueError("arXiv metadata has no entry")
    parsed_by_key: Dict[str, Dict[str, str]] = {}
    for entry_block in entry_blocks:
        id_match = re.search(r"(?is)<id\b[^>]*>(.*?)</id>", entry_block)
        if id_match is None:
            raise ValueError("arXiv metadata has no id")
        entry_url = " ".join(
            html.unescape(re.sub(r"(?s)<[^>]+>", " ", id_match.group(1))).split()
        )
        entry_id_match = re.fullmatch(
            r"https?://arxiv\.org/abs/(.+)",
            entry_url,
        ...
        parsed_by_key[key] = _arxiv_atom_entry(
            entry_block.encode("utf-8"), expected_arxiv_id=expected_id
        )
    if set(parsed_by_key) != set(expected_by_key):
        raise ValueError("arXiv metadata is missing a requested entry")
    return [
        parsed_by_key[_arxiv_base_id_key(arxiv_id)]
        for arxiv_id in normalized_expected
    ]
...
def _arxiv_html_plain_text(raw: bytes) -> str:
    ...
    source = re.sub(
        r"(?is)<(?:script|style|noscript)\b[^>]*>"
        r".*?</(?:script|style|noscript)>",
        " ",
        source,
    )
    source = re.sub(
        r"(?i)</(?:address|article|aside|blockquote|br|dd|div|dl|dt|"
        r"figcaption|figure|footer|h[1-6]|header|hr|li|main|nav|ol|p|pre|"
        r"section|table|tbody|td|th|thead|tr|ul)\s*>",
        "\n",
        source,
    )
...
def _arxiv_locator_excerpts(
    text: str, *, locator: str, maximum_bytes: int
    ...
    if not positions:
        tokens = [
            token
            for token in re.findall(r"[A-Za-z0-9.]+", locator.casefold())
            if len(token) >= 3
        ]
        ...
        excerpts = [
            ...
            for index, (_score, _offset, paragraph) in enumerate(scored[:3])
        ]
    ...
    for index, position in enumerate(positions):
        ...
        # arXiv commonly renders a theorem label as its own paragraph and the
        # statement as the next paragraph.  Do not stop at the first paragraph
        # boundary after the locator: retain the bounded forward context that
        # makes a heading locator useful as primary-source evidence.
        ...
        excerpts.append(
            {
                "occurrence": index + 1,
                "start_char": left,
                "end_char": left + len(excerpt_text),
                "text": excerpt_text,
            }
        )
...
def read_arxiv_primary(
    arxiv_id: str,
    locator: str,
    max_excerpt_bytes: int = 20_000,
    latest_initial_submission_date: Optional[str] = None,
    metadata_endpoint: str = ARXIV_API_URL,
    html_endpoint_template: str = ARXIV_HTML_URL_TEMPLATE,
    timeout_seconds: int = 30,
) -> Dict[str, Any]:
    ...
    cutoff = _normalize_iso_date(
        latest_initial_submission_date,
        label="latest_initial_submission_date",
    )
    if (
        isinstance(max_excerpt_bytes, bool)
        or not isinstance(max_excerpt_bytes, int)
        or not 1_000 <= max_excerpt_bytes <= MAX_ARXIV_PRIMARY_EXCERPT_BYTES
    ):
        raise ValueError(
            "max_excerpt_bytes must be an integer between 1000 and "
            f"{MAX_ARXIV_PRIMARY_EXCERPT_BYTES}"
        )
    if html_endpoint_template.count("{arxiv_id}") != 1:
        raise ValueError("arXiv HTML endpoint template must contain {arxiv_id} once")

    envelope: Dict[str, Any] = {
        "schema_version": "rethlas_arxiv_primary_excerpt_v1",
        "provider": "arxiv_official_html_v1",
        "arxiv_id": normalized_id,
        "locator": normalized_locator,
        "latest_initial_submission_date": cutoff,
        "metadata_endpoint": metadata_endpoint,
        "source_url": html_endpoint_template.format(arxiv_id=normalized_id),
        "primary_source_text": False,
        "mathematical_evidence_authority": False,
        "fallback_used": False,
        "excerpts": [],
    }
    ...
    try:
        metadata_raw = _fetch_bounded_external_bytes(
            metadata_endpoint,
            params={"id_list": normalized_id},
            timeout_seconds=timeout_seconds,
            maximum_bytes=MAX_ARXIV_METADATA_BYTES,
        )
        metadata = _arxiv_atom_entry(
            metadata_raw,
            expected_arxiv_id=normalized_id,
        )
    except _ExternalResponseTooLarge:
    ...
    if cutoff is not None and metadata["initial_submission_date"] > cutoff:
        return {
            **dated_envelope,
            ...
            "error": "initial_submission_after_cutoff",
        }

    try:
        html_raw = _fetch_bounded_external_bytes(
            envelope["source_url"],
            timeout_seconds=timeout_seconds,
            maximum_bytes=MAX_ARXIV_HTML_BYTES,
        )
        plain_text = _arxiv_html_plain_text(html_raw)
        locator_found, match_kind, excerpts = _arxiv_locator_excerpts(
            plain_text,
            locator=normalized_locator,
    ...
    result = {
        **dated_envelope,
        "retrieval_status": "ok",
        "primary_source_text": True,
        "source_sha256": hashlib.sha256(html_raw).hexdigest(),
        "source_utf8_bytes": len(html_raw),
...
def verify_proof_service(
    ...
    profile_name = os.getenv("RETHLAS_MODEL_POLICY_PROFILE", "compatible")
    profile_kwargs: Dict[str, Any] = {"timeout": min(5.0, timeout_seconds)}
    api_token = os.getenv("VERIFY_API_TOKEN")
    if api_token:
        profile_kwargs["headers"] = {"Authorization": f"Bearer {api_token}"}
    profile_response = requests.get(_profile_endpoint(endpoint), **profile_kwargs)
    profile_response.raise_for_status()
    try:
        profile = profile_response.json()
    except ValueError as exc:
        raise ValueError("verification service profile returned non-JSON") from exc
    passes = _validate_verifier_profile(profile, expected_profile=profile_name)
    pass_identity, attempt_id = _verification_pass_identity(
        statement=statement,
        proof_digest_value=proof_digest(proof),
        context_digest=expected_context_digest,
        checked_item_ids=expected_ids,
        verifier_profile=profile_name,
        verifier_service_version=profile["service_version"],
        verifier_pass=passes[0],
        pass_index=1,
    )
    caller_instance_id, caller_pid, caller_start_sha256 = (
        _verification_caller_binding()
    )
    payload = {
        "statement": statement,
    ...
    result = validate_service_response(
        body,
        ...
        expected_verification_pass_index=1,
    )
    if (
        result["verifier_model"] != passes[0]["model"]
        or result["verifier_reasoning_effort"] != passes[0]["reasoning_effort"]
        or result["verifier_service_version"] != profile["service_version"]
    ):
        raise ValueError("verification result differs from selected profile")
    return result
...
def _statement_bound_arxiv_cutoff(
    problem_id: str,
    *,
    ...
    if (
        ...
        and statement_digest != expected_statement_sha256
    ):
    ...
    explicitly_denied = any(
        marker in statement.casefold() for marker in denial_markers
    )
    ...
    matches = re.findall(
        r"(?i)(?:initial submission date is later than|submitted after)\s+"
        r"([0-9]{4}-[0-9]{2}-[0-9]{2})",
        body,
    )
    normalized = {
        _normalize_iso_date(value, label="statement arXiv cutoff")
...
def search_arxiv_theorems_for_problem(
    ...
    cutoff = _statement_bound_arxiv_cutoff(
        problem_id,
    ...
    if not isinstance(raw_results, list):
        return {
            **envelope,
            ...
            "results": [],
            "retrieval_status": "error",
            "error": "search_results_not_a_list",
        }
    if len(raw_results) > num_results:
        return {
            **envelope,
            ...
            "results": [],
            "retrieval_status": "error",
            "error": "search_results_exceed_requested_count",
        }
        ...
        if not isinstance(item, dict) or not isinstance(item.get("arxiv_id"), str):
            return {
                **envelope,
                ...
                "results": [],
                "retrieval_status": "error",
                "error": f"invalid_result:{index}:arxiv_id",
            }
        try:
    ...
    try:
        metadata_raw = _fetch_bounded_external_bytes(
            ARXIV_API_URL,
            params={"id_list": ",".join(metadata_request_ids)},
            timeout_seconds=30,
            maximum_bytes=MAX_ARXIV_METADATA_BYTES,
        )
        metadata_entries = _arxiv_atom_entries(
            metadata_raw,
            expected_arxiv_ids=metadata_request_ids,
        )
    except _ExternalResponseTooLarge:
        ...
        blocked = 0
        metadata_by_id = {
            _arxiv_base_id_key(arxiv_id): metadata
            for arxiv_id, metadata in zip(
                metadata_request_ids, metadata_entries
            )
        }
            ...
            verified.append(
                {
                    **{key: str(value) for key, value in item.items()},
                    "metadata_arxiv_id": metadata["metadata_arxiv_id"],
                    "initial_submission_date": metadata[
    ...
    # Metadata is the admission gate, so a provider or parse failure must
    # return no theorem snippet at all rather than leaking an uncut result.
    return {
        **envelope,
        "count": 0,
        "results": [],
    ...
    try:
        return verify_blueprint_file(
            ...
            publication_state_root=(
                GENERATION_CONTROL_ROOT / "publication_transactions"
            ),
        )
...
def _validate_route_terminal_memory_record(
    channel: str, record: Mapping[str, Any]
) -> None:
    ...
    if set(record) != required:
        raise ValueError(
            "route terminal report must use its exact released schema"
        )
    if channel not in {"proof_steps", "failed_paths"}:
        raise ValueError(
            "route terminal channel must be proof_steps or failed_paths"
        )
    ...
    if thread_id != normalized_thread_id:
        raise ValueError(
            "route terminal thread_id must already use its normalized "
            "route-local form"
        )
    plan_id = record.get("plan_id")
    if (
        not isinstance(plan_id, str)
        or _GENERATION_CONTROL_EVIDENCE_ID_RE.fullmatch(plan_id) is None
    ):
        raise ValueError(
            "route terminal plan_id must match [A-Za-z0-9_.:-]{1,128}"
        )
    if record.get("status") not in {"candidate", "partial", "blocked"}:
        raise ValueError(
            "route terminal status must be candidate, partial, or blocked"
        )
    ...
    if report_text != report_text.strip():
        raise ValueError(
            "route terminal report_text must not have boundary whitespace"
        )
    report_bytes = report_text.encode("utf-8")
    if len(report_bytes) > MAX_ROUTE_TERMINAL_REPORT_BYTES:
        raise ValueError(
            "route terminal report_text exceeds the "
            f"{MAX_ROUTE_TERMINAL_REPORT_BYTES}-byte limit"
        )
    if (
        not isinstance(record.get("report_sha256"), str)
        or record["report_sha256"] != hashlib.sha256(report_bytes).hexdigest()
    ):
        raise ValueError("route terminal report_sha256 does not match report_text")
    _round_text_list(
        record["remaining_obligations"],
        label="route terminal remaining obligations",
    )
    _round_text_list(
        record["decisive_stuck_points"],
        label="route terminal decisive stuck points",
    )
...
def _normalize_route_terminal_thread_id(thread_id: Any) -> str:
    ...
    if thread_id.startswith(_NATIVE_ROOT_THREAD_PREFIX):
        if os.getenv("RETHLAS_RUNTIME_PROFILE") != "legacy":
            raise ValueError(
                "direct /root/<route-local-id> normalization is available "
                "only in the legacy runtime"
            )
        thread_id = thread_id[len(_NATIVE_ROOT_THREAD_PREFIX):]
    if _GENERATION_CONTROL_EVIDENCE_ID_RE.fullmatch(thread_id) is None:
        raise ValueError(
            "route terminal thread_id must be a route-local id or direct "
            "/root/<route-local-id> canonical id matching "
            "[A-Za-z0-9_.:-]{1,128}"
        )
    return thread_id


def _memory_batch_input_preflight(
    problem_id: str,
    items: List[Dict[str, Any]],
    *,
    trusted_control_publication: bool,
    ...
    try:
        if (
            ...
            or sanitize_problem_id(problem_id) != problem_id
        ):
            raise ValueError(
        ...
        for index, item in enumerate(items):
            ...
            ) and not trusted_control_publication:
                raise ValueError(
                    f"{channel} is reserved for trusted control publication"
                )
            if not isinstance(record, dict):
        ...
        raise MemoryCheckpointPreflightError(
            str(exc), retry_allowed=True
        ) from exc
...
def memory_append_batch(
    ...
    *,
    ...
    normalized_items, encoded = _memory_batch_input_preflight(
        problem_id,
        items,
        trusted_control_publication=_trusted_control_publication,
    )
...
def append_route_terminal_report(
    problem_id: str,
    thread_id: str,
    plan_id: str,
    status: str,
    ...
    if not isinstance(report_text, str):
        raise MemoryCheckpointPreflightError(
            "route terminal report_text must be a string",
            retry_allowed=True,
        )
    normalized_report = report_text.strip()
    if not normalized_report:
        raise MemoryCheckpointPreflightError(
            "route terminal report_text must be non-empty",
            retry_allowed=True,
        )
    try:
        ...
        raise MemoryCheckpointPreflightError(
            "route terminal report_text must be valid UTF-8",
            retry_allowed=True,
        ) from exc
    if len(report_bytes) > MAX_ROUTE_TERMINAL_REPORT_BYTES:
        raise MemoryCheckpointPreflightError(
            "route terminal report_text is "
            f"{len(report_bytes)} UTF-8 bytes; compress the same report to at "
            f"most {MAX_ROUTE_TERMINAL_REPORT_BYTES} bytes",
            retry_allowed=True,
        )

    record = {
        "schema_version": ROUTE_TERMINAL_REPORT_SCHEMA,
...
def memory_search(
    ...
    if len(query_bytes) > MAX_MEMORY_SEARCH_QUERY_UTF8_BYTES:
        raise ValueError(
            "query exceeds MAX_MEMORY_SEARCH_QUERY_UTF8_BYTES"
        )
    if (
    ...
    def build_response(
        ...
        *,
        ...
        for channel in search_channels:
            ...
            channel_returned_chars = sum(
                candidate["chars"] for candidate in returned
            )
            ...
            results_by_channel[channel] = {
                ...
                "omitted_ids": [
                    candidate["record_id"]
                    for candidate in omitted[:omitted_id_limit]
                ],
                "omitted_ids_complete": len(omitted) <= omitted_id_limit,
                "returned_chars": channel_returned_chars,
        ...
        return {
            ...
            "max_chars": max_chars,
            "response_max_utf8_bytes": MAX_MEMORY_SEARCH_RESPONSE_UTF8_BYTES,
            ...
            "omitted_ids": [
                candidate["record_id"]
                for candidate in omitted_matches[:omitted_id_limit]
            ],
            "omitted_ids_complete": len(omitted_matches) <= omitted_id_limit,
    ...
    response = build_response(
        returned_keys,
        omitted_id_limit=MAX_OMITTED_IDS,
        response_limited=False,
    )
    if (
        _memory_search_response_utf8_bytes(response)
        <= MAX_MEMORY_SEARCH_RESPONSE_UTF8_BYTES
    ):
        return response

    # Keep the highest-ranked whole records that fit the transport-safe response
    # envelope. Omitted-id diagnostics are lower value and are admitted only from
    # the remaining byte budget. This does not reinterpret the released
    # ``max_chars``/``returned_chars`` record-content contract.
    initially_returned = [
        candidate
        for candidate in all_matches
        if candidate["key"] in returned_keys
    ]

    def response_for_prefix(
        count: int, *, omitted_id_limit: int
    ) -> Dict[str, Any]:
        selected = {
            candidate["key"] for candidate in initially_returned[:count]
        }
        return build_response(
            selected,
            omitted_id_limit=omitted_id_limit,
    ...
    while low < high:
        ...
        if (
            _memory_search_response_utf8_bytes(candidate_response)
            <= MAX_MEMORY_SEARCH_RESPONSE_UTF8_BYTES
        ):
            low = middle
        else:
    ...
    while omitted_low < omitted_high:
        ...
        candidate_response = response_for_prefix(
            retained_count,
            omitted_id_limit=middle,
        )
        if (
            _memory_search_response_utf8_bytes(candidate_response)
            <= MAX_MEMORY_SEARCH_RESPONSE_UTF8_BYTES
    ...
    response = response_for_prefix(
        retained_count,
        omitted_id_limit=omitted_low,
    )
    if (
        _memory_search_response_utf8_bytes(response)
        > MAX_MEMORY_SEARCH_RESPONSE_UTF8_BYTES
...
def route_review_close(
    *,
    ...
    if body.get("state") == "completed_pending_close":
        ...
        present_cutoff_fields = cutoff_fields & set(body)
        if present_cutoff_fields and present_cutoff_fields != cutoff_fields:
            raise ValueError("official review cutoff commitment is incomplete")
        if not present_cutoff_fields:
            # The first official record and its immutable-cutoff stamp are two
            # durable publications.  A power loss between them must repair the
            # missing stamp from that exact official record, rather than leave
            # an otherwise valid review permanently uncloseable.
            unstamped_receipt = _publication_receipt_for_existing(
                problem_id, record, body
            )
            official_receipt = _ensure_immutable_official_cutoff(
                problem_id=problem_id,
                publication_receipt=unstamped_receipt,
            )
            record, body = _find_review_memory(
                problem_id,
                review_id=review_id,
            ...
            official_receipt = _publication_receipt_for_existing(
                problem_id, record, body
            )
...
def _open_targeted_review_lock(
    *,
    problem_id: str,
    review_id: str,
    request_sha256: str,
    ...
    root = Path(
        os.path.abspath(
            os.fspath(GENERATION_CONTROL_ROOT / "targeted_review_locks")
        )
    )
    ...
    try:
        for part in root.parts[1:]:
            while True:
                try:
                    ...
                    raise ValueError(
                        "targeted review lock root must use non-symlink directories"
                    ) from exc
        ...
        lock_key = hashlib.sha256(
            canonical_json_bytes(
                {
                    "schema_version": "rethlas_targeted_review_lock_identity_v1",
                    "problem_id": problem_id,
                    "review_id": review_id,
                    "request_sha256": request_sha256,
                    "snapshot_sha256": snapshot_sha256,
        ...
        lock_name = f"targeted-review-{lock_key}.lock"
        ...
        try:
            ...
            lock_fd = os.open(
                lock_name, os.O_RDWR | nofollow, dir_fd=directory_fd
            )
        ...
        if (
            ...
            os.close(lock_fd)
            raise ValueError("targeted review lock file is unsafe")
        os.fsync(lock_fd)
        os.fsync(directory_fd)
        return directory_fd, lock_fd, root / lock_name
    except BaseException:
        os.close(directory_fd)
        raise
...
def verify_review_claim(
    *, review_id: str, request_sha256: str, snapshot_sha256: str
    ...
    """Serialize one exact targeted-review state machine across processes."""

    problem_id = validate_verified_problem_id(
        _required_review_env("RETHLAS_EXPECTED_PROBLEM_ID", label="review problem id")
    )
    directory_fd, lock_fd, _lock_path = _open_targeted_review_lock(
        problem_id=problem_id,
        review_id=review_id,
        request_sha256=request_sha256,
        snapshot_sha256=snapshot_sha256,
    ...
    try:
        ...
        current_fd = os.open(
            Path(_lock_path).parent,
            os.O_RDONLY | os.O_DIRECTORY | getattr(os, "O_NOFOLLOW", 0),
        )
        try:
            current = os.fstat(current_fd)
            if (held.st_dev, held.st_ino) != (current.st_dev, current.st_ino):
                raise ValueError("targeted review lock root changed while waiting")
        finally:
            os.close(current_fd)
        return _verify_review_claim_locked(
            review_id=review_id,
            request_sha256=request_sha256,
            snapshot_sha256=snapshot_sha256,
        )
...
def _verify_review_claim_locked(
    *, review_id: str, request_sha256: str, snapshot_sha256: str
) -> Dict[str, Any]:
    ...
    targeted = body.get("targeted_verification")
    if targeted is not None and (
        not isinstance(targeted, dict) or targeted.get("ticket") != ticket
    ):
        raise ValueError("durable targeted verification ticket binding changed")

    state = targeted.get("state") if isinstance(targeted, dict) else None
    if state == "completed":
    ...
    if state == "dispatching" and not isinstance(
        targeted.get("verification_deadline_utc"), str
    ):
        # Records emitted before the durable client journal existed cannot be
        # distinguished from an already-sent request and must remain unknown.
    ...
    if state not in {None, "prepared", "dispatching"}:
        raise ValueError("durable targeted verification attempt state is invalid")

    deadline_raw = (
        targeted.get("verification_deadline_utc")
    ...
    if deadline_raw is None:
        ...
        deadline_raw = admission["verification_deadline_utc"]
        prepared_body = deepcopy(body)
        prepared_body["targeted_verification"] = {
            "state": "prepared" if state is None else state,
            "ticket": ticket,
            "attempt": 1,
            "retry_allowed": False,
            "verification_receipt": None,
            "error_sha256": None,
            "verification_deadline_utc": deadline_raw,
        }
    ...
    deadline_raw = _validate_canonical_utc_timestamp(
        deadline_raw, label="targeted verification deadline"
    )
    deadline = datetime.fromisoformat(deadline_raw)
    remaining_seconds = int((deadline - datetime.now(timezone.utc)).total_seconds())

    dispatch_marked = state == "dispatching"

    def mark_verifier_dispatch() -> None:
        nonlocal body, dispatch_marked, record
        if dispatch_marked:
            return
    ...
    try:
        receipt = verify_targeted_claim_service(
            ...
            timeout_seconds=max(1, min(3600, remaining_seconds)),
            api_token=os.getenv("VERIFY_API_TOKEN") or None,
            journal_root=(
                GENERATION_CONTROL_ROOT / "targeted_verifications"
            ),
            on_verifier_dispatch=(
                None if dispatch_marked else mark_verifier_dispatch
            ),
            recover_only=(dispatch_marked or remaining_seconds <= 0),
        )
        if not dispatch_marked:
            # Test doubles and older compatible clients may not invoke the
            # dispatch callback.  Preserve the durable server-side ordering
            # before consuming their concrete result.
            mark_verifier_dispatch()
    except Exception as exc:
        if bool(getattr(exc, "local_retryable", False)):
            raise
        predispatch = bool(getattr(exc, "predispatch", False))
        ambiguous = bool(
            getattr(exc, "outcome_state", None) == "execution_unknown"
            or (
            isinstance(exc, requests.RequestException)
            and getattr(exc, "response", None) is None
            )
        )
        if ambiguous and not dispatch_marked and not predispatch:
            mark_verifier_dispatch()
        if predispatch and state == "dispatching":
            ambiguous = True
        outcome_state = "execution_unknown" if ambiguous else "operational_blocked"
        error_sha = getattr(exc, "error_sha256", None)
        if not isinstance(error_sha, str) or SHA256_RE.fullmatch(error_sha) is None:
...
def _validate_authoritative_handoff(content: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    pending = _authoritative_handoff_pending(problem_id)
    if normalized["schema_version"] == "rethlas_continuous_context_handoff_v1":
        if normalized["pending"] != pending:
            raise ValueError("continuous context handoff lost pending control state")
        return normalized
    last_review, yellow_streak, route_frozen, expected_route = (
        _authoritative_handoff_review_state(problem_id)
    )
...
def _resolved_cost_gate_policy() -> Dict[str, str]:
    raw = os.getenv(_RESOLVED_COST_POLICY_JSON_ENV)
    digest = os.getenv(_RESOLVED_COST_POLICY_SHA256_ENV)
    if raw is None and digest is None:
        manifest = {
            "schema_version": _RESOLVED_COST_POLICY_SCHEMA,
            "policy": "owner_gated",
            "authority": "compatibility_default",
        }
        return {
            **manifest,
            "manifest_sha256": hashlib.sha256(
    ...
    if raw is None or digest is None or os.getenv("RETHLAS_COST_GATE_POLICY") is not None:
        raise ValueError("resolved cost policy manifest transport is incomplete")
    manifest = _strict_json_loads(raw, label="resolved cost policy manifest")
    if (
        not isinstance(manifest, dict)
        or set(manifest) != {"schema_version", "policy", "authority"}
        or manifest.get("schema_version") != _RESOLVED_COST_POLICY_SCHEMA
        or manifest.get("policy") not in {"owner_gated", "disabled_by_owner"}
        or manifest.get("authority") != "owner_wrapper"
        or canonical_json_bytes(manifest).decode("utf-8") != raw
        or hashlib.sha256(raw.encode("utf-8")).hexdigest() != digest
    ):
        raise ValueError("resolved cost policy manifest is invalid")
    return {
...
def _round_text_list(
    value: Any, *, label: str, maximum_items: int = 16
) -> List[str]:
    if (
...
def continuous_round_status(problem_id: str) -> Dict[str, Any]:
    """Read the host manifest used to bind terminal reports to proof lanes."""

    normalized_problem_id = validate_verified_problem_id(problem_id)
    expected_problem_id = validate_verified_problem_id(
        _required_review_env(
            "RETHLAS_EXPECTED_PROBLEM_ID", label="round status problem id"
        )
    )
    if normalized_problem_id != expected_problem_id:
        raise ValueError("round status problem id differs from the active run")
    return _adapter_continuous_round_status(problem_id=normalized_problem_id)


def continuous_round_finish(
    problem_id: str,
    report_record_ids: List[str],
    synthesis_record_id: str,
) -> Dict[str, Any]:
    """Validate and host-seal one terminal three-route cohort."""

    normalized_problem_id = validate_verified_problem_id(problem_id)
    expected_problem_id = validate_verified_problem_id(
        _required_review_env(
            "RETHLAS_EXPECTED_PROBLEM_ID", label="round finish problem id"
        )
    )
    if normalized_problem_id != expected_problem_id:
        raise ValueError("round finish problem id differs from the active run")
    report_ids = _bounded_unique_record_ids(
        report_record_ids, label="round report record ids", maximum=3
    )
    if len(report_ids) != 3:
        raise ValueError("round finish requires exactly three report record ids")
    if (
        not isinstance(synthesis_record_id, str)
        or _GENERATION_CONTROL_EVIDENCE_ID_RE.fullmatch(synthesis_record_id) is None
        or synthesis_record_id in report_ids
    ):
        raise ValueError("round finish synthesis record id is invalid")
    active = _active_memory_records_by_id(normalized_problem_id)
    missing = sorted(set([*report_ids, synthesis_record_id]) - set(active))
    if missing:
        raise ValueError("round finish evidence is not active memory: " + ", ".join(missing))
    ...
    for record_id in report_ids:
        ...
        if (
            entry.get("channel") not in {"proof_steps", "failed_paths"}
            ...
            or record.get("schema_version") != ROUTE_TERMINAL_REPORT_SCHEMA
            or not isinstance(record.get("thread_id"), str)
            or _GENERATION_CONTROL_EVIDENCE_ID_RE.fullmatch(record["thread_id"])
            is None
            or not isinstance(record.get("plan_id"), str)
            or _GENERATION_CONTROL_EVIDENCE_ID_RE.fullmatch(record["plan_id"])
            is None
            or record.get("status") not in {"partial", "blocked"}
            or not isinstance(record.get("report_text"), str)
            or not record["report_text"].strip()
            or record["report_text"] != record["report_text"].strip()
            or len(record["report_text"].encode("utf-8"))
            > MAX_ROUTE_TERMINAL_REPORT_BYTES
            or not isinstance(record.get("report_sha256"), str)
            or record["report_sha256"]
            != hashlib.sha256(record["report_text"].encode("utf-8")).hexdigest()
            or not isinstance(item.get("batch_id"), str)
            or _CONTENT_ADDRESSED_BATCH_ID_RE.fullmatch(item["batch_id"]) is None
        ):
            raise ValueError("round finish route report is malformed")
        _round_text_list(
            record["remaining_obligations"], label="round remaining obligations"
        )
        _round_text_list(
            record["decisive_stuck_points"], label="round decisive stuck points"
        )
        normalized_reports.append(
            {
                ...
                "record_sha256": hashlib.sha256(
                    canonical_json_bytes(
                        {"channel": entry["channel"], "record": record}
                    )
                ).hexdigest(),
            }
        )
    normalized_reports.sort(key=lambda value: value["thread_id"])
    if (
        len({item["thread_id"] for item in normalized_reports}) != 3
        or len({item["plan_id"] for item in normalized_reports}) != 3
    ):
        raise ValueError("round finish reports repeat a thread or plan")

    synthesis_entry = active[synthesis_record_id]
    synthesis_item = synthesis_entry.get("item")
    synthesis = (
        synthesis_item.get("record") if isinstance(synthesis_item, dict) else None
    )
    synthesis_report_ids = (
        synthesis.get("route_report_record_ids")
        if isinstance(synthesis, dict)
        else None
    )
    synthesis_plan_ids = (
        synthesis.get("failed_plan_ids") if isinstance(synthesis, dict) else None
    )
    if (
        synthesis_entry.get("channel") != "failed_paths"
        or not isinstance(synthesis_item, dict)
        or not isinstance(synthesis_item.get("batch_id"), str)
        or _CONTENT_ADDRESSED_BATCH_ID_RE.fullmatch(synthesis_item["batch_id"])
        is None
        ...
        or synthesis.get("schema_version") != ROUND_FAILURE_SYNTHESIS_SCHEMA
        or synthesis.get("record_type") != "key_failures_summary"
        or not isinstance(synthesis_report_ids, list)
        or sorted(synthesis_report_ids) != sorted(report_ids)
        or not isinstance(synthesis_plan_ids, list)
        or sorted(synthesis_plan_ids)
        != sorted(item["plan_id"] for item in normalized_reports)
        or synthesis.get("next_state")
        not in {"new_cohort", "advisor_checkpoint", "stop_unsolved"}
        or not isinstance(synthesis.get("plan_failures"), list)
    ):
        raise ValueError("round finish failure synthesis is malformed")
    plan_failures = synthesis["plan_failures"]
    if (
        len(plan_failures) != 3
        or any(
            not isinstance(item, dict)
            or set(item) != {"plan_id", "stuck_points"}
            or item.get("plan_id")
            not in {report["plan_id"] for report in normalized_reports}
            for item in plan_failures
        )
        or len({item["plan_id"] for item in plan_failures}) != 3
    ):
        raise ValueError("round finish plan failure coverage is invalid")
    for item in plan_failures:
        _round_text_list(item["stuck_points"], label="round plan stuck points")
    _round_text_list(synthesis["common_failures"], label="round common failures")
    _round_text_list(
        synthesis["implications_for_next_plans"],
        label="round implications for next plans",
    )
    normalized_synthesis = {
        "record_id": synthesis_record_id,
        "batch_id": str(synthesis_item["batch_id"]),
        "failed_plan_ids": sorted(synthesis_plan_ids),
        "record_sha256": hashlib.sha256(
            canonical_json_bytes(
                {"channel": synthesis_entry["channel"], "record": synthesis}
            )
        ).hexdigest(),
    }
    # The outer record digest commits report_sha256 and report_text. Preserve
    # the v1 evidence digest while sending report_sha256 separately for the
    # host-lane equality check.
    committed_reports = [
    ...
    evidence_material = {
        ...
        "next_action": synthesis["next_state"],
    }
    evidence_sha256 = hashlib.sha256(
        canonical_json_bytes(evidence_material)
    ).hexdigest()
    return _adapter_continuous_round_finish(
        problem_id=normalized_problem_id,
...
def _validate_generation_wait_evidence(
    ...
    *,
    ...
    resolved_cost_policy = _resolved_cost_gate_policy()
    branch_bound = False
    event_bound = False
    for entry in cited:
        ...
        if state == "waiting_cost_gate":
            event_bound = event_bound or (
                ...
                and (
                    resolved_cost_policy["authority"] != "owner_wrapper"
                    or (
                        isinstance(record.get("orchestration_cost"), dict)
                        and record["orchestration_cost"].get("cost_gate_policy")
                        == resolved_cost_policy["policy"]
                        and record["orchestration_cost"].get(
                            "cost_gate_policy_manifest_sha256"
                        )
                        == resolved_cost_policy["manifest_sha256"]
                    )
                )
            )
        else:
    ...
    if state == "waiting_cost_gate" and resolved_cost_policy["policy"] != (
        "owner_gated"
    ):
        raise ValueError("resolved cost policy forbids waiting_cost_gate")
...
def _checkpoint_failure_tool_result(exc: MemoryCheckpointPreflightError) -> Any:
    ...
    payload = {
        "schema_version": MEMORY_CHECKPOINT_FAILURE_SCHEMA,
        "status": "preflight_failed",
        "category": "memory_checkpoint_validation",
        "error": detail,
...
def build_mcp_app() -> Optional[Any]:
    ...
    @_guarded_tool("read_arxiv_primary")
    def _tool_read_arxiv_primary(
        problem_id: str,
        arxiv_id: str,
        locator: str,
        max_excerpt_bytes: int = 20_000,
    ) -> Dict[str, Any]:
        """Read a bounded official arXiv primary-source excerpt."""
    ...
    @_guarded_tool("continuous_round_status")
    def _tool_continuous_round_status(problem_id: str) -> Dict[str, Any]:
        """Read exact host lane ids and terminal digests for the latest cohort."""
        return continuous_round_status(problem_id=problem_id)

    @_guarded_tool("continuous_round_finish")
    def _tool_continuous_round_finish(
        problem_id: str,
        report_record_ids: List[str],
        synthesis_record_id: str,
    ) -> Dict[str, Any]:
        """Host-seal three durable terminal reports and their synthesis."""
        return continuous_round_finish(
            problem_id=problem_id,
            report_record_ids=report_record_ids,
